...
class RelationsGoldenRecordTaskDb(
    ...
    class BusinessPartnerRelations(
        ...
        @ElementCollection(fetch = FetchType.LAZY)
        @CollectionTable(
            name = "relation_task_states",
            joinColumns = [JoinColumn(name = "relation_id", foreignKey = ForeignKey(name = "fk_task_relation_states_relation"))],
            indexes = [Index(name = "idx_relation_task_states_relation_id", columnList = "relation_id")]
        )
